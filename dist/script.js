...
const accordion = triggersSelector => {
  ...
  btns.forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      btns.forEach(btn => {
        if (!btn.classList.contains('active-class')) {
  ...
  //         content.classList.add('animated', 'backInDown');
  ...
  //         showContent(btn.nextElementSibling);
  //     } else {
  //         hideContent(btn.nextElementSibling);
  //     }
  //    });
  // });
...
const calc = (size, material, options, promo, result) => {
  ...
  const calcFunction = () => {
    ...
    if (!sizeBlock.value || !materialBlock.value) {
      ...
      resultBlock.textContent = `Итоговая сумма: ${Math.round(sum * 0.7)}`;
    } else {
      resultBlock.textContent = `Итоговая сумма: ${sum}`;
    }
  };
...
const drop = () => {
  ...
  function highlight(item) {
    item.closest('.file_upload').style.border = '3px dotted green';
...
const showMoreStyles = (trigger, wrapper) => {
  ...
  btn.addEventListener('click', function () {
    (0,_services_requests__WEBPACK_IMPORTED_MODULE_0__.getResource)('http://localhost:3000/styles').then(res => createCards(res)).catch(() => failCards());
    this.remove();
  });
  function failCards() {
